...
if (trgtColl && daysToRmove && monthsToKeep) {
    var firstLineDate = db[trgtColl].find({creation_time:{$exists:1}}).sort({creation_time:1}).limit(1).next().creation_time.getTime(); //first cdr time  in millisecs
    ...
    for(; i >= 0  && (targetDate-i) < upperLimitDateMili; i-=millisecToAdvance) {
        ...
        let sectionEndCursor = db[trgtColl].find({creation_time:{$lt:new Date(horizon)}}).sort({creation_time:-1}).limit(1);
        print("Horizon  set to :" + new Date(horizon));
        if(!sectionEndCursor.hasNext()) {
            continue;
        }
        var sectionEnd = sectionEndCursor.next();
        var cursor = db[trgtColl].find({creation_time:{$lt:new Date(horizon),$lte:sectionEnd.creation_time}}).sort({creation_time:1}).limit(1);
        while(cursor.hasNext()) {
            var end = cursor.next();
            var linesCount = db[trgtColl].count({creation_time:{$lt:new Date(horizon)},_id:{$lte:end._id}});
            print("Removing :" + linesCount + " lines before the date of : " +  new Date(horizon)+ " with id smaller then :" + end._id);
            var removedResults = db[trgtColl].remove({creation_time:{$lt:new Date(horizon)},_id:{$lte:end._id}});
            if(removedResults.nRemoved) {
                totalRemoved += removedResults.nRemoved;
            }
            printjson(totalRemoved);
            sleep(interval);
            cursor = db[trgtColl].find({creation_time:{$lt:new Date(horizon)},_id:{$lte:sectionEnd._id}}).sort({_id:1}).skip(chunkSize).limit(1);
        }
