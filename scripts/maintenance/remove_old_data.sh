...
# 	mongo --host $DB_HOST --authenticationDatabase=admin -u$DB_USER -p$DB_PASSWORD $DB_NAME  /tmp/data_removal_config.js ./remove_old_cdr_data.js
done

echo " Starting to remove old  invoices/events"
for coll in billrun bills events ; do
	echo "var trgtColl='$coll'; var daysToRmove = $DAYS_TO_REMOVE_PER_ITER; var monthsToKeep = $MONTHS_TO_KEEP;" > /tmp/data_removal_config.js
# 	mongo --host $DB_HOST --authenticationDatabase=admin -u$DB_USER -p$DB_PASSWORD $DB_NAME  /tmp/data_removal_config.js ./creation_time_based_data_removal.js
done
