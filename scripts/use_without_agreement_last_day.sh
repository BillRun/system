...
if [ -n "$OUTPUT" ]; then
    ...
    echo -e "$HEADER\n$OUTPUT" > "/tmp/$FILENAME"
    echo "Report is attached as  $FILENAME" | mail -s "Roaming use without agreement from NRTRDE last day" -a "/tmp/$FILENAME" gminkov@golantelecom.co.il ZOHAROZ@cellcom.co.il yakovep@cellcom.co.il skushnir@golantelecom.co.il mwahab@golantelecom.co.il sdahan@golantelecom.co.il rgolan@golantelecom.co.il eran.uzan@billrun.com
fi
