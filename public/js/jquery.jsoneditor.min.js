...
l(a[d]))&&r(g);g.append(h).append(j);f.append(g);h.val(d).attr("title",d);var m=p(a[d]);j.val(m).attr("title",m);s(g,a[d]);h.change(u(b));j.change(v(b));h.click(w(b));if(k(a[d])||l(a[d]))q(b,a[d],g,(e?e+".":"")+d)}if(k(a)||l(a))d=c("<div>",{"class":"item appender"}),g=c("<button></button>",{"class":"property"}),g.text("Add"),d.append(g),f.append(d),g.click(function(){if(l(a))a.push(null);else if(k(a)){for(var c=1,d="newKey";a.hasOwnProperty(d);)d="newKey"+c,c++;a[d]=null}q(b,a,f,e);b.onchange(n(p(b.original)))})}
