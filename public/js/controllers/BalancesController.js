...
function BalancesController($controller, Utils, $http, $window, Database, $routeParams) {
	...
	vm.saveBalance = function () {
		...
		if (vm.action === "new") {
			...
			else if (_.result(vm.entity.balance, "totals.call.usagev"))
				value = vm.entity.balance.totals.call.usagev;
			else if (_.result(vm.entity.balance, "totals.call.cost"))
				value = vm.entity.balance.totals.call.cost;
			else if (_.result(vm.entity.balance, "totals.sms.usagev"))
				value = vm.entity.balance.totals.sms.usagev;
			else if (_.result(vm.entity.balance, "totals.sms.cost"))
				value = vm.entity.balance.totals.sms.cost;
			else if (_.result(vm.entity.balance, "totals.data.usagev"))
				value = vm.entity.balance.totals.data.usagev;
			else if (_.result(vm.entity.balance, "totals.data.cost"))
				value = vm.entity.balance.totals.data.cost;
			postData.upsert = JSON.stringify({
				value: value,
				expiration_date: vm.entity.to,
