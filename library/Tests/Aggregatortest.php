<?php
 ...
 class Tests_Aggregator extends UnitTestCase {
     ...
     protected $tests = array(
         ...
         array('test' => array('test_number' => 2, "aid" => 3, 'function' => array('basicCompare', 'invoice_exist', 'lineExists', 'foreign', 'passthrough'), 'invoice_path' => '201805_3_101.pdf', 'options' => array('generate_pdf' => 1, "stamp" => "201805", "force_accounts" => array(3))),
             'expected' => array('foreign' => ['TT' => ['subscriber' => ['sid' => 4]]], 'billrun' => array('invoice_id' => 101, 'billrun_key' => '201805', 'aid' => 3),
...
////			)),'expected_invoice'
         ...
         //subscriber with future installment - will bill only for this month 
         array('test' => array('test_number' => 41, "aid" => 84, 'sid' => 85, 'function' => array('basicCompare', 'subsPrice', 'lineExists', 'linesVSbillrun', 'rounded'), 'options' => array("stamp" => "202002",
                     "force_accounts" => array(84))),
             'expected' => array('billrun' => array('invoice_id' => 137, 'billrun_key' => '202002', 'aid' => 84, 'after_vat' => array("85" => 9.36))),
             'line' => array('types' => array('flat', 'service')),
         ),
         //subscriber with future installment -**subscriber is closed during the cycle** - will bill  for all futur installments
         array('test' => array('test_number' => 42, "aid" => 86, 'sid' => 87, 'function' => array('basicCompare', 'subsPrice', 'lineExists', 'linesVSbillrun', 'rounded'), 'options' => array("stamp" => "202002",
                     "force_accounts" => array(86))),
             'expected' => array('billrun' => array('invoice_id' => 138, 'billrun_key' => '202002', 'aid' => 86, 'after_vat' => array("87" => 28.08))),
             'line' => array('types' => array('flat', 'service')),
         ),
         //2 subscribers with future installment -**only one of them (89) is closed during the cycle** - will bill  for all futur installments and sid 90 will bill only for that cycle 
         array('test' => array('test_number' => 43, "aid" => 88, 'sid' => [89, 90], 'function' => array('basicCompare', 'subsPrice', 'lineExists', 'linesVSbillrun', 'rounded'), 'options' => array("stamp" => "202002",
                     "force_accounts" => array(88))),
             'expected' => array('billrun' => array('invoice_id' => 139, 'billrun_key' => '202002', 'aid' => 88, 'after_vat' => array("89" => 28.08, "90" => 9.36))),
             'line' => array('types' => array('flat', 'service')),
         ),
         array(
             'preRun' => ('expected_invoice'),
             'test' => array('test_number' => 43),
             'expected' => array(),
         ),
//		/* run full cycle */
         array(
             'preRun' => ('changeConfig'),
             'test' => array('test_number' => 44, 'aid' => 0, 'function' => array('fullCycle'), 'overrideConfig' => array('key' => 'billrun.charging_day.v', 'value' => 1), 'options' => array("stamp" => "201806", "page" => 0, "size" => 10000000,)),
             'expected' => array(),
         )
     );
      ...
      * @param Mongodloid_Entity|array $returnBillrun is the billrun object of current test after aggregation 
      * @param array $row current test case
      * @return boolean true if the test is pass and false if the test is fail
      */
      ...
      * @return boolean true if the test is pass and false if the tast is fail
      */
     public function foreign($key, $returnBillrun, $row) {
         $lines = $this->getLines($row);
         $expected_foreign = $row['expected']['foreign'];
         $passed = true;
         foreach ($lines as $line) {
             if (array_key_exists($line['key'], $expected_foreign)) {
                 foreach ($expected_foreign[$line['key']] as $key => $val) {
                     if (isset($line['foreign'][$key])) {
                         foreach ($val as $k => $v) {
                             if (isset($line['foreign'][$key][$k])) {
                                 if ($line['foreign'][$key][$k] == $v) {

                                     $this->message .= "in line key {$line['key']} foreign filed from entity  <b>$key</b> : is $k = $v $this->pass";
                                 } else {
                                     $this->message .= "in line key {$line['key']} foreign filed $k form entity $key get wrong value  $this->fail";
                                     $passed = false;
                                 }
                             } else {
                                 $this->message .= "in line key {$line['key']} foreign filed $k form entity $key  isn't create for the line  $this->fail";
                                 $passed = false;
                             }
                         }
                     } else {
                         $this->message .= "in line key {$line['key']} foreign filed for $key  isn't create for the line  $this->fail";
                         $passed = false;
                     }
                 }
             }
