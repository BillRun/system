...
////			)),'expected_invoice'
	...
	public function testMultiDay($key, $returnBillrun, $row) {
		...
		$billruns = $this->getBillruns($query);
		$billruns_ = [];
		foreach ($billruns as $bill) {
			$billruns_[] = $billruns->getRowData();
		}

		if ($this->allow_premature_run) {
			if ($billruns_) {
				$this->message .= "the cycle runs also for values that do not exist in the invoicing day command" . $this->fail;
				$passed = false;
			} else {
				$this->message .= "the cycle runs only for values that exist in the invoicing day command" . $this->pass;
			}
		} else {
			//not_allow_premature_run
			$a = 1;
		}
	}
