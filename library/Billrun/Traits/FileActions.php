	...
	protected function lockFileForReceive($filename, $type, $more_fields = array(), $orphan_window = false) {
		...
		try {
			...
			if ($e->getCode() == Mongodloid_General::DUPLICATE_UNIQUE_INDEX_ERROR) {
				Billrun_Factory::log("Billrun_Traits_FileActions::lockFileForReceive - Trying to relock  a file the was already been locked : " . $filename . " with stamp of : {$logData['stamp']}", Zend_Log::DEBUG);
			} else {
				throw $e;
			}
			return FALSE;
