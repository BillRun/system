...
class Billrun_Processor_Nsn extends Billrun_Processor_Base_Binary {
	...
	public function __construct($options = array()) {
		...
		$this->save_block_header = intval(Billrun_Util::getIn($this->nsnConfig, 'constants.save_block_header', 0));
		...
		while (!$this->isProcessingFinished()) {
			...
			if (in_array($header['format_version'], $this->nsnConfig['block_config']['supported_versions'])) {
				do {
					...
					//Billrun_Factory::log()->log("Last parsed bytes length: " . $this->parser->getLastParseLength(), Billrun_Log::TRACE);
