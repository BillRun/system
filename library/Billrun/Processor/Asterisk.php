...
/**
 * Description of Asterisk
 *
 * @author eran
 */
class Billrun_Processor_Asterisk extends Billrun_Processor_Base_SeparatorFieldLines {

	static protected $type = 'asterisk';
	...
	protected function isValidDataRecord($dataLine) {
		return isset($dataLine['urt']); //preg_match( $this->structConfig['config']['valid_data_line'], );
	}
	...
	protected function buildData($line, $line_number = null) {
		...
		if (isset($row[$this->structConfig['config']['date_field']]) && $row[$this->structConfig['config']['date_field']]) {
