 ...
 * @since    2.8
 */
abstract class Billrun_Exporter_Base extends Billrun_Base {
	...
	protected function loadConfig() {
		$configPath = preg_replace('/^\./',APPLICATION_PATH,Billrun_Util::getIn($this->options,'configByType.exporter.config_path',''));
	...
	protected function getRecordData($row) {
		...
		$baseData = $this->mapFields($fieldsMapping['base'],$row);
		return array_replace_recursive($baseData,$this->buildLineFromStructure($fieldsMapping['custom'], $row));
	}



	protected function buildLineFromStructure($structure, $row, $params = null) {
        $line = array();
        foreach ($structure as $field) {
			if(!empty($field['conditions']) &&
				//Do  all condition met for the  row? (filtering negative the conditions doesn't have any failue)
				!empty($res = array_filter($field['conditions'],
					function($cnd) use ($row) {
						return !Billrun_Util::isConditionMet($row,$cnd);
					}))) {
					continue;
            }
            if (!isset($field['path'])) {
                $message = "Exporter " . $this->getFileType() . " structure is missing a path";
