...
class Billrun_Exporter_Tap3_Tadig extends Billrun_Exporter_Asn1 {
	...
	public function __construct($options = array()) {
		...
		$this->startTime = time();
		$this->isProd = isset($options['prod']) ? $options['prod'] : false;
	...
	protected function getRecEntityInfoList() {
		...
		foreach ($this->recEntities as $recEntity) {
			$ret[] = [
				'RecEntityInformation' => $recEntity,
			];
		}
		return $ret;
	}
	...
	protected function getRecEntityInformation($row) {
		switch ($this->getLineType($row)) {
			case self::$LINE_TYPE_DATA:	
				...
				$recEntityId = Billrun_Util::getIn($row, 'ggsn_address_used', Billrun_Util::getIn($row, 'ggsn_address', ''));
				$recEntityCode = $this->getRecEntityCodeByRecEntityId($recEntityId, $recEntityType);
				break;
	...
	protected function getRecEntityCodeByRecEntityId($recEntityId, $recEntityType) {
		if (!isset($this->recEntities[$recEntityId])) {
			$this->recEntities[$recEntityId] = [
                'RecEntityCode' => count($this->recEntities),
                'RecEntityType' => $recEntityType,
                'RecEntityId' => $recEntityId,
            ];
		}
		return $this->recEntities[$recEntityId]['RecEntityCode'];
	}
	...
	protected function getRecEntityCodeList($row, $fieldMapping) {
		return array(
			...
			array(
				'RecEntityCode' => $this->getSgsnRecEntityInformation($row)['RecEntityCode'],
			),
		);
	}
