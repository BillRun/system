...
//		$cache = Billrun_Factory::cache();
		$timeBoundingQuery = Billrun_Utils_Mongo::getDateBoundQuery(Billrun_Util::getFirstValueIn($row,Billrun_Util::getFieldVal($foreignFieldConfig['query_time_fields'],[]), $row['urt'])->sec);
		switch ($entityType) {
			case 'subscriber' :
				if(empty($row['sid'])) {
					return null;
