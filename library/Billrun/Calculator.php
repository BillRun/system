...
//	}
	...
	public function removeFromQueue() {
		...
		if ((!empty($this->garbageQueueLines)) || (!empty($stamps))){	
			$stamps_remove_from_queue = array_merge($this->garbageQueueLines, $stamps);
		...
		do {
			...
			if ($this->limit != 0) {
				...
				if (isset($querydata['hint'])) {
					$hq = $queue->query($query)->cursor()->setReadPreference(MongoClient::RP_PRIMARY)->hint(array($querydata['hint'] => 1))->limit($this->limit);
					if ($this->autosort) {
						$hq->sort(array('urt' => 1));
					}
				} else {
					$hq = $queue->query($query)->cursor()->setReadPreference(MongoClient::RP_PRIMARY)->limit($this->limit);
					if ($this->autosort) {
						$hq->sort(array('urt' => 1));
					}
				}
			...
			$foundLines = $queue->query(array_merge($localquery, array('hash' => $this->workHash, 'calc_time' => $this->signedMicrotime)))->cursor()->setReadPreference(MongoClient::RP_PRIMARY);
