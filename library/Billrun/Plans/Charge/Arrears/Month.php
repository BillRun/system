...
class Billrun_Plans_Charge_Arrears_Month extends Billrun_Plans_Charge_Base {
	...
	protected function setMonthlyCover() {
		...
		$isTerminated = min( (empty($this->subscriberDeactivation) ? PHP_INT_MAX : $this->subscriberDeactivation), $this->cycle->end()) <  $this->cycle->end();
		$adjustedDeactivation = (empty($this->deactivation) || (!$this->proratedEnd || !$this->proratedTermination && $isTerminated ) ? $this->cycle->end() : $this->deactivation - 1);
