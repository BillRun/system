...
class Billrun_Calculator_Rate_Nsn extends Billrun_Calculator_Rate {
	...
	protected function getLineRate($row, $usage_type) {
		...
		$matchedRate = false;
		$processNoneVoLTE =  Billrun_Factory::config()->getConfigValue('Rate_Nsn.calculator.process_none_VoLTE',false);
		...
		} else if ($processNoneVoLTE && $record_type == '30' && isset($row['ild_prefix'])) {
			$called_number = preg_replace('/^016/', '', $called_number);
			$matchedRate = $this->getRateByParams($called_number, $usage_type, $line_time, $ocg);
		} else if ( $processNoneVoLTE && $record_type == "31" //STC call
