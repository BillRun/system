...
class Billrun_Calculator_Tax_Usage extends Billrun_Calculator_Tax {
	 ...
	 * @param array $line
	 */
	public function getRowTaxData(&$line) {
		if (isset($line['tax_data'])) {
			return $line['tax_data'];
		}
		Billrun_Factory::log()->log("fetching tax data for row {$line['stamp']}", Zend_Log::DEBUG);
		...
		Billrun_Factory::log()->log("finished fetching tax data for row {$line['stamp']}", Zend_Log::DEBUG);
	...
	public static function getTaxByKey($key, $time = null) {
		...
		if (empty($tax)) {
			Billrun_Factory::log()->log("DB lookup for tax key '{$key}'", Zend_Log::DEBUG);
			...
			Billrun_Factory::log()->log("finished DB lookup for tax key '{$key}'", Zend_Log::DEBUG);
