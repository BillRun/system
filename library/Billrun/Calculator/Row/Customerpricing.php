...
//			return $pricingData;
	...
	protected function getRealUsagev($lineToRebalance) {
		...
		foreach ($usedUsagevFields as $usedUsagevField) {
			$usagev += !is_null(Billrun_util::getIn($this->row['uf'], $usedUsagevField)) ? Billrun_util::getIn($this->row['uf'], $usedUsagevField) : 0;
		}
	...
	protected function getLineAprice() {
		...
		if (!is_null(Billrun_util::getIn($userFields, $apriceField)) && is_numeric(Billrun_util::getIn($userFields, $apriceField))) {
