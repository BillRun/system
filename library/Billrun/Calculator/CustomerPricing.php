...
class Billrun_Calculator_CustomerPricing extends Billrun_Calculator {
	...
	protected function getLinePricingData($volume, $usageType, $rate, $sub_balance, $plan) {
		...
		if ($plan->isRateInBasePlan($rate, $usageType)) {
			...
			if ($volumeToCharge < 0) {
				$volumeToCharge = 0;
				$ret['in_plan'] = $volume;
				$accessPrice = empty(['rates'][$usageType]['access_price_out_of_package']) ?  0 : $accessPrice ;
	...
	public function writeLine($line, $dataKey) {
		...
		$save = array();
		$saveProperties = array($this->pricingField, 'billrun', 'over_plan', 'in_plan', 'out_plan', 'plan_ref', 'usagesb', 'arategroup', 'over_arate', 'over_group', 'in_group', 'in_arate', 'vf_count_days', 'roaming_balances', 'addon_balances', 'plan_usage','vf_addon_days');
