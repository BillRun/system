...
class Billrun_Calculator_CustomerPricing extends Billrun_Calculator {
		...
		try {
			...
			if (isset($volume)) {
				if ($row['type'] == 'credit') {
					...
					if (isset($row['freeze_fraction'])) {							
						$pricingData['aprice'] = $pricingData['aprice'] * $row['freeze_fraction'];
					}
				} else {
	...
	public function writeLine($line, $dataKey) {
		...
		$save = array();
		$saveProperties = array($this->pricingField, 'billrun', 'over_plan', 'in_plan', 'out_plan', 'plan_ref', 'usagesb', 'arategroup', 'over_arate', 'over_group', 'in_group', 'in_arate', 'vf_count_days');
