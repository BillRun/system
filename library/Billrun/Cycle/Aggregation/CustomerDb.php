	...
	if (is_null($page)) {
		...
		foreach($billableResults as $revision) {
			...
			if(!in_array($revision['aid'],$this->exclusionQuery)) {
				$revStamp = @Billrun_Util::generateArrayStamp($revision, $customIDFields);
				if(empty($retResults[$revStamp])) {
					$retResults[$revStamp] = [];
				}
				//If the invoice aggregation  is  not a onetime invoice  then  added  the required subscriber plan details
