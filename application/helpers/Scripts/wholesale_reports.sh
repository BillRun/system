...
nsn_end_code='.result.forEach(function(obj) { print("'$year'-'$month'-" + day + "," + (!isNaN(parseInt(obj._id.c,10)) ? parseInt(obj._id.c,10) : obj._id.c ) + "," +( obj._id.r ? db.rates.findOne(obj._id.r.$id).key : "") + "," + obj.count + "," + obj.usagev);});}';
...
nsn_grouping_out='{$group:{_id:{c:"$out_circuit_group",r:"$arate"}, count:{$sum:1},usagev:{$sum:"$usagev"}}}';
nsn_grouping_in='{$group:{_id:{c:"$in_circuit_group",r:"$pzone"}, count:{$sum:1},usagev:{$sum:"$usagev"}}}';
...
#	js_code="$js_code$nsn_end_code" ;;
	...
	"all_out_call" )
	js_code=$js_code'db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", $or:[{record_type:"01"},{record_type:"11", in_circuit_group_name:/^RCEL/},{record_type:"12",in_circuit_group_name:/^BICC/}], out_circuit_group_name:/^(?!RCEL|FCEL|VVOM|BICC)/ }},'$nsn_grouping_out')';
	js_code="$js_code$nsn_end_code" ;;

	"all_nr_out_call" )
	js_code=$js_code'db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"11",in_circuit_group_name:/^RCEL/ }},'$nsn_grouping_out')';
	js_code="$js_code$nsn_end_code" ;;

	"all_nr_in_call" )
	js_code=$js_code'db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"12",out_circuit_group_name:/^RCEL/ }},'$nsn_grouping_in')';
	js_code="$js_code$nsn_end_code" ;;
