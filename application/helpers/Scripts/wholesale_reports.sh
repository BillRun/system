...
case $report_name in
	...
	"data" )
	js_code='db.getMongo().setReadPref("secondaryPreferred");var start_day = 1; var end_day = 31;for(var i = start_day; i <= end_day; i++) {var day = (i.toString().length==1 ? "0" + i : i);var from_date = ISODate("'$year'-'$month'-" + day + "T00:00:00+02:00");var to_date = ISODate("'$year'-'$month'-" + day + "T23:59:59+02:00");db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"ggsn"}},{$group:{_id:{sgsn_address:"$sgsn_address"}, count:{$sum:1},usagev:{$sum:"$usagev"}}}).result.forEach(      function(obj) {  var prvdr = obj._id.sgsn_address;  prvdr = (prvdr.match(/37.26/) ? "GOLAN" : (prvdr.match(/62.90/) ? "NR" : "OTHER"));    print("'$year'-'$month'-" + day + "," + prvdr + "," + obj.count + "," + obj.usagev);});}' ;;

	"all_in_call" )
	js_code='db.getMongo().setReadPref("secondaryPreferred");var start_day = 1; var end_day = 31;for(var i = start_day; i <= end_day; i++) {var day = (i.toString().length==1 ? "0" + i : i);var from_date = ISODate("'$year'-'$month'-" + day + "T00:00:00+02:00");var to_date = ISODate("'$year'-'$month'-" + day + "T23:59:59+02:00");db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"11", in_circuit_group:/^(?!RCEL|FCEL|VVOM|PCT)/ , out_circuit_group_name:/^(?!VVOM|PCT)/}},{$group:{_id:{icg:"$in_circuit_group"}, count:{$sum:1},usagev:{$sum:"$usagev"}}}).result.forEach(      function(obj) {         print("'$year'-'$month'-" + day + "," + obj._id.icg + "," + obj.count + "," + obj.usagev);});}' ;;

	"all_out_call" )
	js_code='db.getMongo().setReadPref("secondaryPreferred");var start_day = 1; var end_day = 31;for(var i = start_day; i <= end_day; i++) {var day = (i.toString().length==1 ? "0" + i : i);var from_date = ISODate("'$year'-'$month'-" + day + "T00:00:00+02:00");var to_date = ISODate("'$year'-'$month'-" + day + "T23:59:59+02:00");db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"12",out_circuit_group_name:/^(?!RCEL|FCEL|VVOM|PCT)/ ,in_circuit_group:/^(?!VVOM|PCT)/ }},{$group:{_id:{ocg:"$out_circuit_group"}, count:{$sum:1},usagev:{$sum:"$usagev"}}}).result.forEach(      function(obj) {         print("'$year'-'$month'-" + day + "," + obj._id.ocg + "," + obj.count + "," + obj.usagev);});}' ;;

	"all_nr_out_call" )
	js_code='db.getMongo().setReadPref("secondaryPreferred");var start_day = 1; var end_day = 31;for(var i = start_day; i <= end_day; i++) {var day = (i.toString().length==1 ? "0" + i : i);var from_date = ISODate("'$year'-'$month'-" + day + "T00:00:00+02:00");var to_date = ISODate("'$year'-'$month'-" + day + "T23:59:59+02:00");db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"11",in_circuit_group_name:/^RCEL/ ,out_circuit_group:/^(?!FCEL|VVOM|PCT)/ }},{$group:{_id:"NR_IN", count:{$sum:1},usagev:{$sum:"$usagev"}}}).result.forEach(      function(obj) {         print("'$year'-'$month'-" + day + "," + obj._id + "," + obj.count + "," + obj.usagev);});}' ;;

	"all_nr_in_call" )
	js_code='db.getMongo().setReadPref("secondaryPreferred");var start_day = 1; var end_day = 31;for(var i = start_day; i <= end_day; i++) {var day = (i.toString().length==1 ? "0" + i : i);var from_date = ISODate("'$year'-'$month'-" + day + "T00:00:00+02:00");var to_date = ISODate("'$year'-'$month'-" + day + "T23:59:59+02:00");db.lines.aggregate({$match:{urt:{$gte:from_date, $lte:to_date}, type:"nsn", record_type:"12",out_circuit_group_name:/^RCEL/ ,in_circuit_group:/^(?!FCEL|VVOM|PCT)/ }},{$group:{_id:"NR_OUT", count:{$sum:1},usagev:{$sum:"$usagev"}}}).result.forEach(      function(obj) {         print("'$year'-'$month'-" + day + "," + obj._id + "," + obj.count + "," + obj.usagev);});}' ;;
