...
class fraudAlertsPlugin extends Billrun_Plugin_BillrunPluginBase {
	...
	protected function roamingNotify($eventsCol, $linesCol) {
		...
		$events = $eventsCol->aggregate(array('$match' => array('notify_time' => array('$exists' => false),
				'source' => array('$in' => array('nrtrde', 'ggsn')))), array('$sort' => array('imsi' => 1)), array('$group' => array(
				'_id' => '$imsi',
		...
		foreach ($events as $event) {
			if ($this->notifyOnEvent($event)) {
				...
				$eventsCol->update(array('notify_time' => array('$exists' => false),
					...
					), array('$set' => array('notify_time' => time())), array('multiple' => 1));

				//mark deposit for the lines on the current imsi 
				$linesCol->update(array('process_time' => array('$lt' => date('Y-m-d H:i:s', $this->startTime), 'imsi' => $event['imsi'])), array('$set' => array('deposit_stamp' => $event['id'])), array('multiple' => 1));
				$retValue[] = $event;
			}
		}
		return $retValue;
