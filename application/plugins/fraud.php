...
//		$events = Billrun_Factory::db()->eventsCollection();
		...
		$properties = Billrun_Factory::config()->getConfigValue('fraud.usage_types', array());
		...
		if ($usage_type == 'data') {
			foreach ($usage_type_properties[$usage_type]['threshold_usagev'] as $threshold_plan => $threshold_name) {
				...
				if ($this->over_threshold($value_before, $value, $threshold_value)) {
		...
		} else if (over_threshold($value_before, $value, $threshold)) {
		...
		if ($this->over_threshold($value_before, $value, $threshold)) {
			$this->insert_fraud_event('cost', $value, $value_before, $row, $threshold, 'NIS', 'FP_NATIONAL1', $fraud_connection, $fraud_connection_options);
		}
	}

	protected function insert_fraud_event($filter_by, $value, $value_before, $row, $threshold, $units, $event_type, $fraud_connection, $fraud_connection_options) {
